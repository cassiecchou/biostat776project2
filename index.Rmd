...
ggplot(data = df %>% filter(year >= 2014), aes(x = date, y = temperature)) + geom_point(aes(color = temp_type)) + facet_wrap(~city_name, scales = "free_y") + labs(title = "Temperature over Time, Sorted by City", color = "Temperature Type") + xlab("Date") + ylab("Temperature")
